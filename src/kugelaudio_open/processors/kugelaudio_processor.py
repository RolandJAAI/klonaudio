...
class KugelAudioProcessor:
    ...
    Supports three voice input modes (in priority order):
    1. voice_cache: Pre-encoded voice features dict
    2. voice: Named pre-encoded voice from voices.json registry
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """Process text and optional voice input for speech synthesis.

        Supports three voice input modes with the following priority:
        1. voice_cache: Pre-encoded voice features (highest priority)
        2. voice: Name of a pre-encoded voice from the voices.json registry
        3. voice_prompt: Raw audio file or tensor for voice cloning

        When voice_prompt is used, the raw audio is processed and passed as
        speech_tensors/speech_masks so the model can encode it through its
        acoustic and semantic tokenizers at inference time.
        ...
        Args:
            ...
            voice_cache: Pre-encoded voice features dict (alternative to voice name).
                Takes priority over voice and voice_prompt.
            voice_prompt: Path to an audio file (str) or a raw audio tensor
                (torch.Tensor) for voice cloning. Used only when neither
                voice_cache nor voice is provided.
        ...
        Returns:
            BatchEncoding with processed inputs. Always includes text_ids and
            speech_input_mask. When voice_cache or voice is provided, includes
            voice_cache. When voice_prompt is provided, includes speech_tensors
            and speech_masks for the model to encode at inference time.
        """
        if text is None:
        ...
        # Priority: voice_cache > voice > voice_prompt
        loaded_voice_cache = voice_cache
        if voice is not None and voice_cache is None:
            loaded_voice_cache = self.load_voice_cache(voice)

        # Process voice_prompt into speech_tensors when no pre-encoded voice is available
        speech_tensors = None
        speech_masks = None
        if voice_prompt is not None and loaded_voice_cache is None:
            # Load and process the audio through the audio processor
            if isinstance(voice_prompt, str):
                if not os.path.exists(voice_prompt):
                    raise FileNotFoundError(
                        f"Voice prompt file not found: {voice_prompt}"
                    )
                # voice_prompt is a file path
                audio_result = self.audio_processor(voice_prompt, return_tensors="pt")
                speech_tensors = audio_result["audio"]  # shape: [1, 1, samples]
            elif isinstance(voice_prompt, torch.Tensor):
                # voice_prompt is already a tensor
                speech_tensors = voice_prompt
                if speech_tensors.dim() == 1:
                    speech_tensors = speech_tensors.unsqueeze(0).unsqueeze(0)  # [1, 1, samples]
                elif speech_tensors.dim() == 2:
                    speech_tensors = speech_tensors.unsqueeze(1)  # [batch, 1, samples]
            else:
                raise ValueError(
            ...
            # Frame count estimation: Must match encoder's actual compression ratio (3200)
            # to ensure speech_input_mask size matches the number of embeddings produced.
            num_samples = speech_tensors.shape[-1]
            num_voice_tokens = math.ceil(num_samples / self.speech_compression_ratio)
            num_voice_tokens = max(num_voice_tokens, 1)

            # Create speech_masks (all frames valid)
            batch_size = speech_tensors.shape[0]
            speech_masks = torch.ones(batch_size, num_voice_tokens, dtype=torch.bool)

        # Process pre-encoded voice or voice_prompt placeholder tokens
        has_voice_input = loaded_voice_cache is not None or speech_tensors is not None
        if has_voice_input:
