...
class KugelAudioForConditionalGenerationInference(KugelAudioPreTrainedModel, GenerationMixin):
    ...
    def encode_voice_prompt(
        ...
        """Encode voice prompt for cloning.

        Args:
            voice_audio: Audio tensor [batch, channels, samples] or file path
            sample_rate: Sample rate of input audio

        Returns:
            Dict with:
                - acoustic_mean: Acoustic encoder output mean
                - acoustic_std: Acoustic encoder output std
                - semantic_mean: Semantic encoder output mean
        ...
        if isinstance(voice_audio, str):
            ...
            audio_processor = AudioProcessor(sampling_rate=sample_rate)
            voice_audio = audio_processor(voice_audio, return_tensors="pt")["audio"]
        ...
        acoustic_output = self.acoustic_tokenizer.encode(voice_audio)
        semantic_output = self.semantic_tokenizer.encode(voice_audio)
    ...
    def _process_speech_inputs(
        ...
        1. voice_cache: Pre-encoded voice features (from encode_voice_prompt)
        2. speech_tensors: Raw audio to encode through both tokenizers
        3. Neither: Returns dummy features

        Args:
            speech_tensors: Raw audio tensor [batch, channels, samples] or None
            speech_masks: Boolean mask indicating valid frames, or None
            voice_cache: Dict with "acoustic_mean", "semantic_mean", and optional "acoustic_std"
        ...
        if voice_cache is not None:
            ...
            if "semantic_mean" in voice_cache:
                ...
                # Fallback: zero semantic features for legacy voice files
                semantic_mean = torch.zeros_like(acoustic_mean)
            ...
            # Guard against stripped encoders
            if getattr(self.acoustic_tokenizer, 'encoder', None) is None:
                raise RuntimeError(
                    "Cannot process raw speech tensors: tokenizer encoders have been stripped. "
                    "Use voice_cache with pre-encoded features instead."
                )
        ...
        # Index combined embeddings by speech_masks
        speech_embeds = combined_embed[speech_masks]
    ...
    def generate(
        ...
        **kwargs,
        ...
        """Generate speech from text using a voice reference.

        Supports three voice input methods:
        1. voice_cache: Pre-encoded voice features (from encode_voice_prompt or .pt files)
        2. speech_tensors/speech_masks: Raw audio tensors from the processor's
           voice_prompt pipeline, to be encoded through the model's tokenizers
        3. voice_prompt: Legacy parameter, equivalent to speech_tensors
        ...
        Args:
            ...
            voice_cache: Pre-encoded voice features (dict with "acoustic_mean" tensor).
                Takes priority over speech_tensors and voice_prompt.
            speech_tensors: Raw audio tensor [batch, channels, samples] to encode
                through the model's acoustic and semantic tokenizers for voice cloning.
                Typically provided by the processor when voice_prompt is used.
            speech_masks: Boolean mask indicating valid frames for speech_tensors.
                If None and speech_tensors is provided, defaults to all-ones mask.
            speech_input_mask: Boolean mask indicating where to insert voice embeddings
                in the input sequence
            voice_prompt: Legacy parameter for raw audio tensor. Maps to speech_tensors
                for backward compatibility. Ignored if speech_tensors is already set.
            cfg_scale: Classifier-free guidance scale (higher = more faithful to text)
            max_new_tokens: Maximum tokens to generate
            do_sample: Whether to sample or use greedy decoding
            temperature: Sampling temperature
            speech_end_penalty: Logit penalty applied to speech_end token to prevent
                premature termination. Higher values make the model less likely to end
                early. Default 5.0. Set to 0.0 to disable.
        ...
        # Handle legacy voice_prompt parameter (maps to speech_tensors)
        if voice_prompt is not None and speech_tensors is None:
            speech_tensors = voice_prompt

        # Note: if speech_tensors is provided without speech_masks, _process_speech_inputs()
        # will create an all-valid mask after encoding. No action needed here.
        ...
        # Process voice inputs if provided (voice_cache, speech_tensors, or voice_prompt)
        if voice_cache is not None or speech_tensors is not None:
            _, speech_embeds = self._process_speech_inputs(
                speech_tensors=speech_tensors.to(device=device, dtype=dtype) if speech_tensors is not None else None,
                speech_masks=speech_masks.to(device=device) if speech_masks is not None else None,
                voice_cache=voice_cache,
            )
        ...
        for step in progress_iter:
            ...
            # Apply speech_end penalty to prevent premature termination
